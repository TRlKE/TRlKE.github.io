...
function building(building) {
    ...
    if (building == 'house' || building == 'maxHouseBtn') {
        if (houses < 1) { // starting sequence
            ...
            setTimeout(() => {
                message("now, 2 settlers have come. Assign them to be farmers or else your village will starve.");
...
setInterval(function() {
    if (maxpopulation > population) {
        if (food < 0) {
            ...
            if (populationRound <= 0 && dead == false) {
                console.log('dead');
                message('Welp, your people starved since you didnt have enough farmers, just reload the page :(');
                dead = true;
            }
        } else {population = population * 1.005};
...
function change(item) {
    ...
    if(item == 'resourcesPerSec') {
        foodPerSec = farmers - (population * 0.5);
        document.getElementById('foodPerSec').innerHTML = Math.round(foodPerSec * 100) / 100;
        woodPerSec = lumberjacks / 2;
        document.getElementById('woodPerSec').innerHTML = Math.round(woodPerSec * 100) / 100;
        stonePerSec = miners / 4;
        document.getElementById('stonePerSec').innerHTML = Math.round(stonePerSec * 100) / 100;
    }
